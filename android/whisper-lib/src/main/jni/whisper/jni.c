...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    // FIX (2026-09-05): the official example hardcodes English here, but
    // this build is specifically for the whisper-small-roman-urdu
    // fine-tune, whose own model card is explicit: "always run with
    // language auto-detect (-l auto), NEVER force -l ur — forcing the
    // native language token leaks native-script (Urdu-script) output;
    // -l auto keeps the transcript in clean Roman/Latin script." Forcing
    // "en" here would risk the same kind of problem in the other
    // direction (fighting the model's own fine-tuning), so this uses
